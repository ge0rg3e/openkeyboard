<script lang="ts">
	import { connect, disconnect, controller, connected, deviceInfo, error, brightness as deviceBrightness, gameMode, macroLed, battery } from '$lib/store';
	...
	<div class="mx-auto flex max-w-7xl flex-col gap-6">
		...
		<header class="flex items-center justify-between gap-3">
			...
			<div class="flex items-center gap-2">
				...
				{:else}
					<Button variant="outline" size="sm" onclick={() => connect()}>Connect Keyboard</Button>
				{/if}
			</div>
		</header>
			...
			<div class="flex flex-wrap items-center justify-between gap-2">
				<div class="flex items-center gap-2">
					...
					{#if $deviceInfo?.serial}
						<span class="hidden text-xs text-muted-foreground sm:inline">Serial {`${$deviceInfo.serial}`}</span>
					{/if}
				...
				{#if $deviceInfo?.firmware}
					<span class="text-xs text-muted-foreground">Firmware {`${$deviceInfo.firmware}`}</span>
				{/if}
			</div>
		{:else}
			<div class="rounded-lg border border-border/60 bg-secondary/50 px-4 py-2.5 text-center text-sm text-muted-foreground">
			...
			<aside class="flex flex-col gap-4">
				<Card class="border-border/60 bg-background/60">
					<CardHeader class="pb-2">
						<CardTitle class="text-xs uppercase tracking-wider text-muted-foreground">Effect</CardTitle>
					</CardHeader>
						...
						{#if showMode}
							<div class="grid grid-cols-3 gap-1.5">
								{#each ['single', 'dual', 'random'] as m}
									<Button variant={breathingMode === m ? 'default' : 'outline'} size="sm" class="text-xs capitalize" onclick={() => (breathingMode = m as typeof breathingMode)}>
										{m}
									</Button>
								{/each}
							</div>
						...
						{#if showColor1}
							<div class="grid grid-cols-2 gap-2">
								<div class="flex flex-col gap-1.5">
									...
									<Input type="color" bind:value={color1} class="h-9 w-full p-1" />
								</div>
								{#if showColor2}
									<div class="flex flex-col gap-1.5">
										<Label class="text-xs">Color 2</Label>
										<Input type="color" bind:value={color2} class="h-9 w-full p-1" />
									</div>
								{/if}
							</div>
						{/if}
						...
						{#if kind === 'custom' && canCustom}
							...
							<div class="flex flex-col gap-2.5">
								<div class="flex flex-col gap-1.5">
									...
									<Input type="color" bind:value={paintColor} class="h-9 w-full p-1" />
								</div>
								...
								<div class="flex gap-1.5">
									<Button variant="outline" size="sm" class="flex-1 text-xs" onclick={clearCustom}>Clear</Button>
								...
								<span class="text-[10px] leading-relaxed text-muted-foreground">
									Click keys on the keyboard to paint them. Per-key lighting is host-rendered and resets when the keyboard powers off.
								</span>
							</div>
						{/if}
							...
							{#if $connected && vendor === 'razer'}
								...
								<div class="flex items-center justify-between gap-3">
									<div class="flex flex-col">
										...
										<span class="text-[10px] text-muted-foreground">Disables the Windows key</span>
									</div>
									<Switch
										checked={gm}
										onCheckedChange={(c) => {
								...
								<div class="flex items-center justify-between gap-3">
									<div class="flex flex-col">
										...
										<span class="text-[10px] text-muted-foreground">M1-M5 backlight</span>
									</div>
									<Switch
										checked={ml}
										onCheckedChange={(c) => {
