...
sudo udevadm trigger`;
  ...
  const activeGroup = $derived(
    categories.find((c) => c.id === activeCategory)!.groups,
  );
  const deviceCount = $derived(
    activeGroup.reduce((n, g) => n + g.items.length, 0),
  );
  ...
  const features = [
    {
      ...
      description:
        "Static, wave, spectrum, reactive, breathing and starlight, rendered by the keyboard itself.",
    },
    {
      title: "No install",
      description:
        "Runs in Chromium browsers over WebHID. No drivers, no Synapse, nothing to download.",
    },
    {
      title: "Privacy first",
      description:
        "Everything stays on your machine and your keyboard. No telemetry no tracking.",
    },
  ];
</script>
...
<div class="flex min-h-screen flex-col bg-background">
  ...
  >
    <div
      class="mx-auto flex w-full max-w-6xl items-center justify-between py-3"
    >
      ...
      <div class="flex items-center gap-3">
        <Button href="#troubleshooting" variant="ghost" size="sm"
          >Troubleshooting</Button
        >
        <Button variant="outline" size="sm" onclick={startConnect}
  ...
  <main class="flex-1">
    <!-- Hero -->
    <section
      class="mx-auto w-full max-w-4xl px-4 pt-16 pb-12 text-center sm:pt-24"
    >
      <h1
      ...
      <p
        class="mx-auto mt-4 max-w-1xl text-base text-muted-foreground text-pretty sm:text-lg"
      >
        A free, open-source WebHID app that controls your keyboard without any
        proprietary software.<br />Connect your keyboard, change its settings,
        and close the web app when you're done.
      </p>

      <div class="mt-8 flex flex-wrap items-center justify-center gap-3">
        <Button size="lg" onclick={startConnect}>Connect Keyboard</Button>
        <Button href="#supported" variant="outline" size="lg">
          Supported keyboards
        </Button>
      </div>
    </section>
    ...
    <section id="supported" class="border-t border-border/60 bg-secondary/40">
      <div class="mx-auto w-full max-w-5xl px-4 py-16">
        <div
          class="flex flex-col items-start justify-between gap-3 sm:flex-row sm:items-end"
        >
          <div>
              ...
              {#each cat.groups as group}
                <div class="mt-10">
                  <div class="flex items-center gap-2">
                    ...
                    <span
                      class="rounded-full bg-secondary px-2 py-0.5 text-[10px] font-medium text-muted-foreground"
                  ...
                  <div class="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                    {#each group.items as d}
                      <div
                        class="flex h-[180px] flex-col overflow-hidden rounded-xl border border-border/60 bg-card transition-colors hover:border-border"
                      >
                        <div class="min-h-0 flex-1 bg-secondary/50">
                          ...
                          {:else}
                            <div
                              class="flex h-full w-full items-center justify-center text-3xl text-muted-foreground/30"
                            >
                              ⌨
                            </div>
                          {/if}
                        ...
                        <div
                          class="shrink-0 truncate border-t border-border/60 px-4 py-2.5 text-sm font-medium"
                        >
                          {d.name}
                        </div>
                      </div>
    ...
    >
      <div class="mx-auto w-full max-w-4xl px-4 py-16">
        ...
        <div class="mt-8 rounded-xl border border-border/60 bg-card p-6">
          ...
          >
            Browsers talk to your keyboard through the WebHID API, which on
            Linux is guarded by the OS.<br />By default your keyboard is only
            readable, not writable, so the app connects but the lighting
            controls do nothing until a small udev rule grants access.
          </p>
          <p
            class="mt-3 max-w-2xl text-sm leading-relaxed text-muted-foreground"
          >
            If "Connect Keyboard" looks like it worked but nothing changes, run
            these three lines in a terminal, then unplug and replug your
            keyboard (or reboot):
          </p>
          <div class="relative mt-4">
            ...
            <pre
              class="overflow-x-auto rounded-lg border border-border/60 bg-background px-4 py-3 font-mono text-xs leading-relaxed text-amber-100">{linuxSetupCommands}</pre>
          </div>
          <a
  ...
  <footer class="border-t border-border/60">
    <div
      class="mx-auto flex w-full max-w-6xl items-center justify-between py-6 text-xs text-muted-foreground"
    >
      ...
      <div class="flex items-center gap-3">
        <Button
          ...
          size="sm"
          class="px-0"
          href="https://github.com/ge0rg3e/openkeyboard">GitHub</Button
        >
        ...
        <Button
          ...
          size="sm"
          class="px-0"
          href="https://x.com/ge0rg3e_dev">Follow @ge0rg3e_dev on X</Button
        >
      </div>
    </div>
  </footer>
